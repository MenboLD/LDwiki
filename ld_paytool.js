(async () => {
  ...
  function getBudgetEl(){ return elBudgetYen || document.getElementById('optBudgetYen'); }
  function getBudgetQuickEl(){ return elBudgetQuick || document.getElementById('btnBudgetQuick'); }
  function getSortEl(){ return elSort || document.getElementById('optSort'); }
  ...
  function buildResourceToggleUI(){
    ...
    if(!elToggles.__bound){
      elToggles.__bound = true;
  ...
  function buildRateEditorUI(){
    ...
    const items = [
      { type:'input', key:'gold', name:'ゴールド', icon:'Resource_01_gold_20x20px.png' },
      { type:'select', key:'mine_key', name:'鍵', icon:'Resource_02_key_20x20px.png', selectId:'optMineKeyRate', options:[420,500,600] },
      { type:'input', key:'battery', name:'バッテリー', icon:'Resource_04_battery_20x20px.png' },
      { type:'input', key:'pet_food', name:'ペットフード', icon:'Resource_05_petfood_20x20px.png' },
      { type:'input', key:'mythic_stone', name:'神話石', icon:'Resource_06_Mythstone_20x20px.png' },
      { type:'input', key:'immortal_stone', name:'不滅石', icon:'Resource_07_immotalstone_20x20px.png' },
      { type:'input', key:'churu', name:'チュール', icon:'Resource_03_chur_20x20px.png' },
      { type:'input', key:'diamond', name:'ダイヤ', icon:'Resource_08_dia_20x20px.png' },
      { type:'input', key:'invite', name:'招待状', icon:'Resource_09_Scroll_20x20px.png' },
    ];
    ...
    for(const it of items){
      ...
      if(it.type === 'select'){
        const current = Number(elMineKeyRate?.value);
        const opts = it.options.map(v => `<option value="${v}" ${v===current?'selected':''}>${v}</option>`).join('');
        row.innerHTML = `
          <img class="pt-rate-icon" src="${baseUrl}${it.icon}" alt="${it.name}">
          <div class="pt-rate-name">${it.name}</div>
          <select id="${it.selectId}" class="pt-select pt-rate-select" aria-label="${it.name}のレート">${opts}</select>
        `;
      }else{
        row.innerHTML = `
          <img class="pt-rate-icon" src="${baseUrl}${it.icon}" alt="${it.name}">
    ...
    if(!elRateEditor.__bound){
      elRateEditor.__bound = true;
      elRateEditor.addEventListener('change', (e)=>{
        ...
        if(t instanceof HTMLSelectElement){
          if(t.id === 'optMineKeyRate'){
            // nothing stored in rateBase; effective rate reads from select
            applyAll();
          }
        }
      });
...
function getEffectiveRates(){
    const r = {...rateBase};
    const mkSel = elMineKeyRate || document.getElementById('optMineKeyRate');
    r.mine_key = Number(mkSel?.value);
    for(const k of RESOURCE_KEYS){
      if(!toggles[k]) r[k] = 0;
    }
    return r;
  ...
  function render(rows){
    ...
    tbody.innerHTML = rows.map(r => {
      ...
      const dpy100 = (r._calc_dpy && r._calc_dpy>0) ? Math.round(r._calc_dpy * 100) : 0;

      return `
      <tr data-key="${key}">
        <td class="sticky-col1 name pt-namecell" title="${r.package_name}"><div class="pt-sticky-cell"><span class="pt-nameText">${fmtName(r.package_name)}</span></div></td>
        <td class="sticky-col2 count">${fmtNum(qty)}/${maxDisp}</td>
        ...
        <td class="calc calc-dia">${fmtNum(r._calc_dia)}</td>
        <td class="calc calc-dpy">${dpy100 ? fmtNum(dpy100) : '-'}</td>
        <td class="calc calc-ratio">${fmtPct1(r._calc_budget_ratio)}</td>
      </tr>`;
    }).join('');
  }
  ...
  function calcAndSort(){
    ...
    const budgetEl = elBudgetYen || document.getElementById('optBudgetYen');
    const budget = clampInt(budgetEl ? budgetEl.value : 8000, 0, 200000);
    ...
    if(elKpi) elKpi.textContent = `基準(予算${budget}) Dia/¥=${budgetDiaPerYen ? (Math.round(budgetDiaPerYen*1000)/1000).toFixed(3) : '-'} / 初回2倍: ${enabled}/${uniq.length}使用可`;
    ...
    const mode = (getSortEl()?.value) || 'default';
    let out = rows.slice();
    switch(mode){
  ...
  function updateSummary(rows, budgetDiaPerYen){
    ...
    const dpy100 = (dpy>0) ? Math.round(dpy * 100) : 0;
    ...
    const totalRow = `
      <tr>
        <td class="sticky-col1 name pt-namecell"><div class="pt-sticky-cell"><span class="pt-nameText">合計</span></div></td>
        <td class="sticky-col2 count">${fmtNum(sumQty)}</td>
        ...
        <td class="calc calc-dia">${fmtNum(sumDia)}</td>
        <td class="calc calc-dpy">${dpy100 ? fmtNum(dpy100) : '-'}</td>
        <td class="calc calc-ratio">${fmtPct1(ratioB)}</td>
      </tr>`;
    ...
    const detailRows = picked.map(({r, qty}) => {
      ...
      const dpy2_100 = (dpy2>0) ? Math.round(dpy2 * 100) : 0;
      ...
      <tr data-key="${rowKey(r)}">
        <td class="sticky-col1 name pt-namecell" title="${r.package_name}"><div class="pt-sticky-cell"><span class="pt-nameText">${fmtName(r.package_name)}</span></div></td>
        <td class="sticky-col2 count">${fmtNum(qty)}</td>
        ...
        <td class="calc calc-dia">${fmtNum(dia)}</td>
        <td class="calc calc-dpy">${dpy2_100 ? fmtNum(dpy2_100) : '-'}</td>
        <td class="calc calc-ratio">${fmtPct1(ratio2)}</td>
      </tr>`;
    }).join('');
...
function applyAll(){
  ...
  try {
    ...
    on(getBudgetEl(), 'blur', applyBudget);
    on(getBudgetEl(), 'keydown', (e)=>{
      if(e.key === 'Enter'){
        e.preventDefault();
        getBudgetEl()?.blur();
        applyBudget();
    ...
    function getBudgetVal(){ const el = getBudgetEl(); return clampInt(el ? el.value : 8000, 0, 200000); }
    function setBudgetVal(v){
      const nv = clampInt(v, 0, 200000);
      const el = getBudgetEl(); if(el) el.value = String(nv);
    ...
    on(getBudgetQuickEl(), 'click', openBudgetPopup);
