(async () => {
  ...
  // --- Step0整理: event listener duplication guard ---
  function bindOnce(el, key, type, handler, options){
    if(!el) return;
    const k = `ptBound_${key}_${type}`;
...
function getEffectiveRates(){
    const r = {...rateBase};
    r.mine_key = elMineKeyRate ? (Number(elMineKeyRate.value) || 0) : (Number(rateBase.mine_key) || 0);
    for(const k of RESOURCE_KEYS){
      if(!toggles[k]) r[k] = 0;
    }
    return r;
...
function applyAll(){
  ...
  try {
    ...
    // Auto recalcs (bound once)
    bindOnce(elMineKeyRate, "mineKey", "change", applyAll);
    bindOnce(elRateEditor, "rateEditor", "input", (e)=>{ pullRatesFromUI(); applyAll(); });
    bindOnce(elRateEditor, "rateEditor", "change", (e)=>{ pullRatesFromUI(); applyAll(); });
    bindOnce(elToggles, "resourceToggles", "change", applyAll);
    bindOnce(elDoubleToggles, "doubleToggles", "change", applyAll);
