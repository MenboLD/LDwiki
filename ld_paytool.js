...
function getEffectiveRates(){
  ...
  function render(rows){
    tbody.innerHTML = rows.map(r => {
      ...
      <tr data-key="${key}">
        ...
        <td class="count pt-countcell"><div class="pt-sticky2"><span class="pt-countText">${qty}/${maxDisp}</span></div></td>
  ...
  function calcAndSort(){
    ...
    const budget = clampInt(elBudgetYen ? elBudgetYen.value : 0, 0, 200000);
  ...
  function updateSummary(rows, budgetDiaPerYen){
    ...
    const totalRow = `
      <tr>
        ...
        <td class="count pt-countcell"><div class="pt-sticky2">${fmtNum(sumQty)}</div></td>
    ...
    const detailRows = picked.map(({r, qty}) => {
      ...
      <tr>
        <td class="name pt-namecell" title="${r.package_name}"><div class="pt-sticky1"><span class="pt-nameText">${fmtName(r.package_name)}</span></div></td>
        <td class="count pt-countcell"><div class="pt-sticky2">${fmtNum(qty)}</div></td>
        ...
        <td class="calc">${(yen>0)?fmtNum(Math.round(dpy2*100)):'-'}</td>
