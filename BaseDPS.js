(() => {
  ...
  function syncSpeedDerivedUI() {
    const oldAspd = readNumber($("aspd") ? $("aspd").value : "0");
    if ($("baseSpd") && !$("baseSpd").value && oldAspd > 0) {
      $("baseSpd").value = toFixed6Display(oldAspd);
    }
    const baseSpd = readNumber($("baseSpd") ? $("baseSpd").value : "0");
    const bowPct = readNumber($("bowPct") ? $("bowPct").value : "0");
    const passivePct = readNumber($("passivePct") ? $("passivePct").value : "0");
    const bowMul = 1 + bowPct / 100;
    const passiveMul = 1 + passivePct / 100;
    const finalSpd = baseSpd * bowMul * passiveMul;
    if ($("aspd")) $("aspd").value = isFinite(finalSpd) ? String(finalSpd) : "0";
    if ($("finalSpdOut")) $("finalSpdOut").value = isFinite(finalSpd) ? finalSpd.toFixed(6) : "0.000000";
  }
  ...
  function toFixed6Display(raw) {
    const t = trimDecimals(raw, 6);
    if (!t) return "";
  ...
  function toPctDisplay6(raw, maxIntDigits = 4) {
    let t = trimDecimals(raw, 6);
    if (!t) return "";
    let [a, b = ""] = t.split(".");
    a = (a || "0").slice(0, maxIntDigits);
    a = String(parseInt(a, 10) || 0);
  ...
  const calcState = { target: null, expr: "", suffix: "" };

  function flashCalcButton(btn) {
  ...
  function renderCalcDisplay() {
    const d = $("calcDisplay");
    if (!d) return;
    const base = calcState.expr || "0";
    d.textContent = calcState.suffix ? `${base}${calcState.suffix}` : base;
  }
  ...
  function applyCalc() {
    if (!calcState.target) return closeCalc();
    let val = 0;
    try { val = safeEvalExpression(calcState.expr || "0"); } catch (_) { val = readNumber(calcState.target.value); }
    const out = calcState.suffix === "%" ? toPctDisplay6(String(val), 4) : toFixed6Display(String(val));
    calcState.target.value = out;
    closeCalc();
    syncSpeedDerivedUI();
    validateAndRender();
  }

  function openCalcForField(el, suffix = "") {
    calcState.target = el;
    calcState.suffix = suffix;
    calcState.expr = String(readNumber(el.value) || 0);
    if (calcState.expr === "0" && !el.value) calcState.expr = "";
    if ($("calcOverlay")) $("calcOverlay").hidden = false;
    renderCalcDisplay();
  }

  function setupCalcModal() {
    document.querySelectorAll(".calcPctField").forEach(el => {
      const open = (e) => { e.preventDefault(); openCalcForField(el, "%"); };
      el.addEventListener("click", open);
      el.addEventListener("focus", open);
    });
    if ($("calcCloseBtn")) $("calcCloseBtn").addEventListener("click", closeCalc);
    if ($("calcCancelBtn")) $("calcCancelBtn").addEventListener("click", closeCalc);
    if ($("calcApplyBtn")) $("calcApplyBtn").addEventListener("click", applyCalc);
    if ($("calcOverlay")) $("calcOverlay").addEventListener("click", (e) => { if (e.target === $("calcOverlay")) closeCalc(); });
    if ($("calcPad")) {
      $("calcPad").querySelectorAll("button").forEach(btn => {
        btn.addEventListener("click", () => {
          flashCalcButton(btn);
          const act = btn.dataset.act;
          if (act === "clear") calcState.expr = "";
          else if (act === "back") calcState.expr = calcState.expr.slice(0, -1);
          else if (act === "eval") {
            try { calcState.expr = String(safeEvalExpression(calcState.expr || "0")); } catch (_) {}
          } else {
            calcState.expr += btn.dataset.val || "";
          }
          renderCalcDisplay();
        });
      });
  ...
  function initBindings() {
    ...
    bindField($("baseSpd"), (v)=>trimDecimalsLive(v,6), (v)=>toFixed6Display(v));
    ...
    $("bowPct").addEventListener("change", () => { $("bowPct").value = toPctDisplay6($("bowPct").value, 4); syncSpeedDerivedUI(); validateAndRender(); });
    $("passivePct").addEventListener("change", () => { $("passivePct").value = toPctDisplay6($("passivePct").value, 4); syncSpeedDerivedUI(); validateAndRender(); });
    ...
    $("baseSpd").addEventListener("input", () => { syncSpeedDerivedUI(); scheduleRender(); });
  ...
  function normalizeAll() {
    ...
    if (!$("baseSpd").value && readNumber($("aspd").value) > 0) {
      $("baseSpd").value = toFixed6Display($("aspd").value);
    }
    $("baseSpd").value = toFixed6Display($("baseSpd").value);
    $("bowPct").value = toPctDisplay6($("bowPct").value, 4);
    $("passivePct").value = toPctDisplay6($("passivePct").value, 4);
  ...
  function getValInternal() {
    ...
    return { atk, aspd, gaugeMax, manaPerSec, envDiff, baseDef, defReduce, realDef, physMul,
      ...
      baseSpd, bowPct, passivePct, basicAttr, basicTarget, aAttr, aTarget, bAttr, bTarget, uAttr, uTarget };
  }

  function clearErrAll() {
    const ids = ["atk","baseSpd","bowPct","passivePct","gaugeMax","manaRegenPct","defReduce","critChancePct","critPhysBonusPct","critMagicBonusPct","ultMulPct","ultF","aMulPct","aPPct","aF","bMulPct","bThird","bF"];
  ...
  function validateInputs() {
    ...
    syncSpeedDerivedUI();
    const baseSpd = readNumber($("baseSpd").value);
    if (!(baseSpd > 0)) { setErr($("baseSpd"), true); errors.push("基礎速度は0より大きい値"); }
    const aspd = readNumber($("aspd").value);
  ...
  function calcTotal(v) {
    ...
    if (v.ultType === "mana") {
      ...
      return { dps, detail: { ...nu, timeManaPerFrame_nonUlt, basicManaPerFrame_nonUlt, manaPerFrame_nonUlt, nonUltDPS_adj, framesNonUltToReady, cycleFrames, cycleDamage } };
    }
    ...
    if (v.ultType === "cool") {
      ...
      const dps = cycleDamage / (cycleFrames / F);
      return { dps, detail: { ...nu, coolPerFrame_nonUlt, nonUltDPS_adj, framesNonUltToReady, cycleFrames, cycleDamage } };
    }
...
function render() {
  ...
  function load(silent = false) {
    try {
      ...
      if (!("baseSpd" in v) && ("aspd" in v)) v.baseSpd = toFixed6Display(v.aspd);
      if (!("bowPct" in v)) v.bowPct = "0%";
      if (!("passivePct" in v)) v.passivePct = "0%";
