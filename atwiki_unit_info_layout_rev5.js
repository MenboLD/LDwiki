/* atwiki_unit_info_layout_rev5.js */
(function () {
  'use strict';
  ...
  function formatBoolCircle(value) {
    if (value === true || value === 'true' || value === 1 || value === '1') return '〇';
  ...
  function highlightSkillDescription(value) {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      parts.push('<span class="ld-unit-info__skill-highlight">' + escapeHtml(match[0]) + '</span>');
  ...
  function prefixedValue(prefix, value, formatter) {
    var body = formatter ? formatter(value) : formatPlain(value);
  ...
  function buildSkillTableHtml(skill) {
    return [
      '<table class="ld-unit-info__skill-table" aria-label="スキル情報">',
      '  <tbody>',
      '    <tr>',
      '      <td class="ld-unit-info__skill-cell ld-unit-info__skill-cell--name" colspan="4">', prefixedValue('スキル名：', skill.skill_name), '</td>',
      '    </tr>',
      '    <tr>',
      '      <td class="ld-unit-info__skill-cell ld-unit-info__skill-cell--desc" colspan="3">', highlightSkillDescription(skill.skill_description), '</td>',
      '      <td class="ld-unit-info__skill-cell ld-unit-info__skill-cell--meta">', prefixedValue('クールタイム：', skill.cooldown_time), '</td>',
      '    </tr>',
      '    <tr>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('属性型：', skill.attribute), '</td>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('範囲型：', skill.range_type), '</td>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('スキル判定：', skill.is_skill, formatBoolCircle), '</td>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('スキル型：', skill.skill_type), '</td>',
      '    </tr>',
      '    <tr>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('起点型：', skill.origin), '</td>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('対象型：', skill.target_type), '</td>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('究極判定：', skill.is_ultimate, formatBoolCircle), '</td>',
      '      <td class="ld-unit-info__skill-cell">', prefixedValue('特性型：', skill.skill_trait_note), '</td>',
      '    </tr>',
      '    <tr>',
      '      <td class="ld-unit-info__skill-cell ld-unit-info__skill-cell--params" colspan="4">', escapeHtml('未記載パラメータ：'), formatLineBreakText(skill.undocumented_parameters), '</td>',
      '    </tr>',
      '  </tbody>',
      '</table>'
    ].join('');
  ...
  function buildSkillsHtml(skillsForUnit) {
    if (!skillsForUnit || !skillsForUnit.length) {
      return '<div class="ld-unit-info__skill-empty">スキル情報がありません。</div>';
    }

    return skillsForUnit.map(function (skill) {
      return buildSkillTableHtml(skill);
    }).join('');
  }

  function renderUnit(root, unit, unitMap, skillGroupMap) {
    var panel = root.querySelector('.ld-unit-info__panel');
    if (!panel) return;

    var skillsForUnit = skillGroupMap.get(String(unit.unit_name || '')) || [];
    panel.innerHTML = [
      '<div class="ld-unit-info__block ld-unit-info__block--unit">',
      buildUnitTableHtml(unit, unitMap),
      '</div>',
      '<div class="ld-unit-info__block ld-unit-info__block--skills">',
      buildSkillsHtml(skillsForUnit),
      '</div>'
    ].join('');
  ...
  function buildSkillGroupMap(skills) {
    var map = new Map();
    sortSkills(skills).forEach(function (skill) {
      var key = String(skill.unit_name || '');
  ...
  async function init() {
    ...
    try {
      ...
      var skillGroupMap = buildSkillGroupMap(skills);

      var currentUnit = getDefaultUnit(units, root);
      fillSelect(root, units, currentUnit);
      renderUnit(root, currentUnit, unitMap, skillGroupMap);
      ...
      if (select) {
        select.addEventListener('change', function () {
          ...
          if (!nextUnit) return;
          renderUnit(root, nextUnit, unitMap, skillGroupMap);
        });
      }
    } catch (error) {
      console.error(error);
