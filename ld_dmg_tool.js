...
function renderTreasureDetail(){
  ...
  const rowsAll = getTreasureRowsFromAtk(unit);
...
function getTreasureRowsFromAtk(unitName){
  const u = String(unitName || '').trim();
  if(!u) return [];
  const rows = (STATE.masters.unitAtk || STATE.masters.unit_atk || STATE.masters.atk || []);
...
function EXISTS_TREASURE_ATK(unitName, unitLevel){
  ...
  return getTreasureRowsFromAtk(unitName).length > 0;
}

function getTreasureCoef(unitName, treasureLv){
  const lvl = String(treasureLv ?? '').trim();
  const rows = getTreasureRowsFromAtk(unitName).filter(r => normAtkTreasureLv(r) === lvl);
