...
function renderTreasureDetail(){
  ...
  const lab = document.querySelector('[data-label-for="treasure_level"]') || document.getElementById('label_treasure_level');
  if(lab){
    lab.textContent = labelText ? `財宝Lv（${labelText}）` : '財宝Lv';
  }
  const on = String(STATE.values.treasure_on) === 'true';
  const lvl = Number(STATE.values.treasure_level || 1);
  const coef = (exists && on) ? getTreasureCoef(unit, lvl) : null;
  STATE.computed = STATE.computed || {};
  STATE.computed.treasure_coef = coef;
  const out = document.getElementById('out_treasure_coef');
  if(out){
    out.textContent = (coef===null) ? '-' : String(coef);
  }
}
...
function applyAllVisibility(){
  ...
  applyTreasureVisibilityOverride();
}
...
function applyTreasureVisibilityOverride(){
  ...
  const exists = treasureExistsForUnit(unit);
  setVisible('treasure_on', exists && ulv >= 10);
  const on = String(STATE.values.treasure_on) === 'true';
  setVisible('treasure_level', (exists && ulv >= 10) && on);
}
...
function hookTreasureRecalc(){
  ...
  ids.forEach(id=>{
    ...
    if(el){
      el.addEventListener('change', ()=>{ try{ applyTreasureVisibilityOverride(); renderTreasureDetail(); updateTreasureDebugLine(); }catch(e){} });
      el.addEventListener('input',  ()=>{ try{ applyTreasureVisibilityOverride(); renderTreasureDetail(); updateTreasureDebugLine(); }catch(e){} });
    }
  });
}
...
function updateTreasureDebugLine(){
  ...
  const c = getTreasureRowsForUnit(unit).length;
  const msg = `treasure rows: ${c} (unit="${unit}")`;
