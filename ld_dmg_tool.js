...
function getTreasureRowsForUnit(unitName){
  ...
  if(!u) return [];
  return rows.filter(r => String((r.treasurename ?? '')).trim() === u);
}
function renderTreasureDetail(){
  ...
  const rows = getTreasureRowsForUnit(unit)
    .filter(r => String((r.parame_1 ?? '')).trim() === lvl);

  if(rows.length === 0){
    const p = document.createElement('div');
    p.className = 'muted';
    p.textContent = '該当データなし（treasurename + parame_1 の一致がありません）';
    host.appendChild(p);
    return;
  }
  for(const r of rows){
    ...
    l.textContent = String(r.parame_text ?? '（名称なし）');
    h.appendChild(l);
    const c = document.createElement('div');
    c.className = 'fieldContent';
    const v = document.createElement('div');
    v.className = 'outVal mono';
    v.textContent = String(r.parame_2 ?? '');
    c.appendChild(v);
    line.appendChild(h);
    line.appendChild(c);
