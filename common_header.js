...
/* common_header_core_20251220u.js
   ...
   - Lock UI: login button shows "ロック中" (white text, red)
              and remaining time is shown on pass button (B案) WITHOUT redundant "ロック中".
   Notes:
   - Supabase config must be provided by supabase_config.js:
       window.LD_SUPABASE_URL / window.LD_SUPABASE_ANON_KEY
*/
(() => {
  'use strict';

  const VERSION = '20251220u';
  ...
  function injectHeader(){
    ...
    const elPage = document.getElementById('topbarPageName');
  ...
  function ensurePassButtonMode(){
    ...
    let btn = document.getElementById('authPassBtn');
    if(!btn){
    ...
    const ghost = document.getElementById('authGhost');
    if(ghost){
  ...
  function setLoggedInUI(username){
    const elLabel = document.getElementById('topbarAuthLabel');
  ...
  function scheduleUserExistsCheck(){
    ...
    userExistsTimer = setTimeout(async () => {
      const latestName = safeTrim((document.getElementById('authUserName') && document.getElementById('authUserName').value) || '');
      ...
      try{
        ...
        const nowName = safeTrim((document.getElementById('authUserName') && document.getElementById('authUserName').value) || '');
  ...
  function updateAuthControls(){
    const elUser = document.getElementById('authUserName');
    const elPass = document.getElementById('authPass');
    const elPassBtn = document.getElementById('authPassBtn');
    const elLoginBtn = document.getElementById('authLoginBtn');
    ...
    // A: username empty -> guest, pass disabled
    if(!user){
      elPass.value = '';
    ...
    if(isLocked){
      ...
      elPassBtn.disabled = true;
      // IMPORTANT: keep it short (no "ロック中")
      elPassBtn.textContent = `残り ${fmtRemain(untilMs - now)}`;
      ensureLockTicker();
      return;
    }

    // Supabase config missing: disable login, but keep UI safe.
    if(!supabaseReady){
  ...
  function setupAuth(){
    ...
    const elForm = document.getElementById('authForm');
    ...
    window.addEventListener('resize', () => {
      const l = document.getElementById('topbarAuthLabel');
      if(!l) return;
