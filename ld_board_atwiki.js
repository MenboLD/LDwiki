(() => {
  ...
  const STORAGE_KEYS = {
    deviceId: 'ldmb_board_device_id_v3',
    name: 'ldmb_board_name_v3'
  };
  ...
  function mount(root) {
    ...
    root.innerHTML = `
      <form id="ldmbp-form" class="ldmbp-form" novalidate>
        <div class="ldmbp-row">
          <input id="ldmbp-name" class="ldmbp-name" type="text" maxlength="24" autocomplete="nickname" placeholder="名前">
  ...
  async function init() {
    ...
    function renderComments(items) {
      ...
      el.list.innerHTML = items.map((item) => `
        <li class="ldmbp-item">
          <div class="ldmbp-time">${escapeHtml(formatDate(item.created_at))}</div>
    ...
    async function onSubmit(event) {
      ...
      el.submit.disabled = true;
      try {
        const deviceIdHash = await sha256Hex(getOrCreateDeviceId());
        ...
        localStorage.setItem(STORAGE_KEYS.name, checked.name);
        el.comment.value = '';
        updateCharCount();
