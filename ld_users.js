...
(() => {
  ...
  const userPassInput = $("userPassInput");
    ...
    // Ensure page pass inputs allow full-width input and are visible (no bullet masking)
    try {
      if (userPassInput) {
        userPassInput.type = 'text';
        userPassInput.setAttribute('inputmode','text');
        userPassInput.removeAttribute('pattern');
      ...
      if (inputPass) {
        ...
        inputPass.removeAttribute('pattern');
