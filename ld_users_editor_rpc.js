  ...
  btn.addEventListener("click", async () => {
    ...
    if (res.ok){
      ...
      if (g.ok){
        ...
        // locked_until が返る場合はローカルにも反映
        if (g.lockedUntilMs && g.lockedUntilMs > Date.now()){
          localStorage.setItem(authState.lockKeyPrefix + usernameRaw, String(g.lockedUntilMs));
          showToast("一定時間ロック中です。", 2200);
          refresh();
          return;
        }

        const k = String(g.kind || "").toLowerCase();
        if (k.includes("pass not set")){
          showToast("このユーザーはパス未設定です（管理者側で pass_hash を設定するか、新規登録してください）。", 3200);
        } else if (k.includes("does not exist") || k.includes("function") || k.includes("rpc_error")){
          showToast("ユーザーデータ取得RPC（ld_get_userdata）が未導入/呼び出し不可です。", 3200);
        } else {
          showToast(`ユーザーデータ取得に失敗しました（${g.kind || "unknown"}）。`, 3200);
        }
      }
      return;
    }
