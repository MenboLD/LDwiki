(function () {
  ...
  function buildMaterialHtml(materialIds, unitMap) {
    ...
    return '<div class="ld-unit-info__materials">' + items.join('') + '</div>';
  }
  ...
  function renderUnit(root, unit, unitMap) {
    ...
    panel.innerHTML = [
      '<div class="ld-unit-info__card">',
      '  <div class="ld-unit-info__table-wrap">',
      '    <table class="ld-unit-info__table" aria-label="ユニット情報">',
      '      <tbody>',
      '        <tr>',
      '          <td class="ld-unit-info__image-cell" rowspan="3">', buildImageHtml(unit), '</td>',
      '          <th scope="row" class="ld-unit-info__label">名称</th>',
      '          <td class="ld-unit-info__name-value" colspan="3">', escapeHtml(unit.unit_name || '-'), '</td>',
      '        </tr>',
      '        <tr>',
      '          <th scope="row" class="ld-unit-info__label">ランク</th>',
      '          <td class="ld-unit-info__value">', escapeHtml(unit.rarity_name || '-'), '</td>',
      '          <th scope="row" class="ld-unit-info__label">攻撃力</th>',
      '          <td class="ld-unit-info__value">', formatInteger(unit.attack_damage), '</td>',
      '        </tr>',
      '        <tr>',
      ...
      '        </tr>',
      '        <tr>',
      '          <td class="ld-unit-info__materials-cell" rowspan="2">', buildMaterialHtml(materialIdsFromUnit(unit), unitMap), '</td>',
  ...
  async function init() {
    ...
    try {
      ...
      var unitMap = new Map(units.map(function (unit) {
      ...
      select.addEventListener('change', function () {
        ...
        var selectedUnit = units.find(function (unit) {
          return toNullableNumber(unit.unit_id) === selectedId;
        });
        if (!selectedUnit) return;
        renderUnit(root, selectedUnit, unitMap);
      });
    } catch (error) {
      console.error(error);
      setStatus(root, '読み込みに失敗しました。RLS または JS 読み込み設定を確認してください。', true);
    }
  }
