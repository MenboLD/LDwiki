...
(() => {
  ...
  let state = { items: [] };
  let filterQuery = '';
  let opVisibleWanted = false; // default false
  ...
  function render() {
    ...
    const rowsHtml = visibleItems.map((it, idx) => {
      ...
      const upDisabled = idx === 0 || !user || searching;
      const downDisabled = idx === visibleItems.length - 1 || !user || searching;
      const topDisabled = idx === 0 || !user || searching;
  ...
  function applyFilterFromInput() {
    ...
    render();
    syncOpVisibility(); // 検索中は操作列を強制非表示
    setStatus(filterQuery ? '検索中（操作は無効）' : 'OK');
  }
  if (noteSearch) {
