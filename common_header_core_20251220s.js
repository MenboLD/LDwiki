...
function injectHeader(){
    ...
    topbar.innerHTML = `
      ...
      <form class="topbar-row topbar-row--auth" id="authForm" aria-label="ログイン操作" autocomplete="on">
        ...
        <button id="authPassBtn" type="button" class="topbar-auth-passbtn" aria-label="パス入力">要パス</button>
...
// Remove legacy footer note (not needed now)
  ...
  function updateAuthControls(){
    ...
    const setGuestState = () => {
      ...
      elPass.value = "";
            const pb = $("authPassBtn"); if(pb) pb.textContent = "要パス";
    ...
    const setNeedPassState = () => {
      ...
      elPassBtn.textContent = (elPass.value && elPass.value.length) ? elPass.value : "要パス";
